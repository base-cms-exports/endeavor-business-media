...
const { amtSectionIds } = require('../amt-section-ids');
...
module.exports = async ({ apollo }) => {
  ...
  // amt Export: Standard || Description || Descriptiong & Logo
  // 3128144 || 3128145 || 3128146
  const companies = retrieveFilterdCompanies(allCompanies, amtSectionIds);
  ...
  const printContent = arr => arr.map((c) => {
    const taxonomyIds = [3128144, 3128145, 3128146];
    ...
    const text = [];
    if (companyTaxonomyIds.includes(3128146) && c.primaryImage) {
